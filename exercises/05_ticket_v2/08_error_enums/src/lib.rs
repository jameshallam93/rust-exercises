...
fn easy_ticket(title: String, description: String, status: Status) -> Ticket {
    match Ticket::new(title.clone(), description.clone(), status.clone()) {
        ...
        Err(error) => {
            match error {
                TicketNewError::TitleError { error_string } => {
                    panic!("{}", error_string)
                },
                TicketNewError::DescriptionError { error_string } => {
...
impl Ticket {
    pub fn new(
        ...
        if title.is_empty() {
            return Err(TicketNewError::TitleError { error_string: ("Title cannot be empty".to_string()) });
        }
        if title.len() > 50 {
            return Err(TicketNewError::TitleError { error_string: ("Title cannot be longer than 50 bytes".to_string())});
        }
        if description.is_empty() {
            return Err(TicketNewError::DescriptionError { error_string: ("Description cannot be empty".to_string())});
        }
        if description.len() > 500 {
            return Err(TicketNewError::DescriptionError { error_string: ("Description cannot be longer than 500 bytes".to_string()) });
        }
